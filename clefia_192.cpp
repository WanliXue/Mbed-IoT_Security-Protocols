...
void KeySchedule192(unsigned char *RoundKey, const unsigned char *InitialKey)
{
    const unsigned char InitialConstant[2] = {0x71U, 0x37U}; // cubic root of 3 
    unsigned char InitialKey192[32];
    unsigned char IntermediateKey[32];
    unsigned char con192[4 * 84];
    int i;
    copy(InitialKey192, InitialKey, 24);
    for(i = 0; i < 8; i++)
    {
        InitialKey192[i + 24] = ~InitialKey[i];
    }
    // Generate CONi^(192) (0 <= i < 84, IntermediateKey = 42) 
    ConstantGenerate(con192, InitialConstant, 42);
    // GFN(8,10) (generating IntermediateKey from InitialKey) 
    GFN_8(IntermediateKey, InitialKey192, con192, 10);
    // Expanding InitalKey and IntermediateKey to generate RoundKey and WhiteningKey
    XOR(RoundKey, InitialKey192, InitialKey192 + 16, 8); // InitialWhiteningKey (WK0, WK1)
    RoundKey += 8;
    for(i = 0; i < 11; i++)
    { 
        if((i / 2) % 2)
        {
            XOR(RoundKey, IntermediateKey + 16, con192 + i * 16 + (4 * 40), 16); // IntermediateKey Right
            if(i % 2)
            {
                XOR(RoundKey, RoundKey, InitialKey192 + 0,  16); //XOR K Left
            }
            DoubleSwap(IntermediateKey + 16); // Update IntermediateRight
        }
        else
        {
            XOR(RoundKey, IntermediateKey + 0,  con192 + i * 16 + (4 * 40), 16); // IntermediateKey Left
            if(i % 2)
            {
                XOR(RoundKey, RoundKey, InitialKey192 + 16, 16); //XOR Key Right
            }
            DoubleSwap(IntermediateKey + 0);  // Update IntermediateLeft
        }
        RoundKey += 16;
    }
    XOR(RoundKey, InitialKey192 + 8, InitialKey192 + 24, 8); // InitialWhiteningKey (WK2, WK3)
}
...
void Encrypt(unsigned char *CipherText, const unsigned char *PlainText, const unsigned char *RoundKey, const int NoRounds)
{
    ...
    RoundKey += 8;
    GFN_4(rout, rin, RoundKey, NoRounds); //GFN{8,10}
...
int main(void)
{
    //Initial Key value: ffeeddccbbaa99887766554433221100f0e0d0c0b0a090807060504030201000
    ...
    int NoRounds = 22; //Number of rounds for 192Bit key
    ...
    for (int temp=1;temp<=32;temp++)
    {
        KeySchedule192(RoundKey, InitialKey);
