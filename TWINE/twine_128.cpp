...
{
    int i;
    uint8_t RoundKey[128/4],temp, temp1, temp2, temp3;
    for(i=0;i<(128/4);i++)
        RoundKey[i] = (key[(i/4)]>>(4*(i&0x03))) & 0x0F;

    for(i=0;i<35;i++)
    {
        output[i][0] = RoundKey[2];output[i][1] = RoundKey[3];
        output[i][2] = RoundKey[12];output[i][3] = RoundKey[15];
        output[i][4] = RoundKey[17];output[i][5] = RoundKey[18];
        output[i][6] = RoundKey[28];output[i][7] = RoundKey[31];
           
        RoundKey[1]=RoundKey[1] ^SBox[RoundKey[0]];
        RoundKey[4]=RoundKey[4] ^SBox[RoundKey[16]];
        RoundKey[23]=RoundKey[23] ^SBox[RoundKey[30]];
        ...
        RoundKey[16]=RoundKey[20];RoundKey[17]=RoundKey[21];
        RoundKey[18]=RoundKey[22];RoundKey[19]=RoundKey[23];
        RoundKey[20]=RoundKey[24];RoundKey[21]=RoundKey[25];
        RoundKey[22]=RoundKey[26];RoundKey[23]=RoundKey[27];
        RoundKey[24]=RoundKey[28];RoundKey[25]=RoundKey[29];
        RoundKey[26]=RoundKey[30];RoundKey[27]=RoundKey[31];
        RoundKey[28]=temp;RoundKey[29]=temp1;
        RoundKey[30]=temp2;RoundKey[31]=temp3;
    }
    output[35][0] = RoundKey[2];output[35][1] = RoundKey[3];
    output[35][2] = RoundKey[12];output[35][3] = RoundKey[15];
    output[35][4] = RoundKey[17];output[35][5] = RoundKey[18];
    output[35][6] = RoundKey[28];output[35][7] = RoundKey[31];
}     
...
int main()
{
    ...
    uint16_t KEY[(128/4)];
    uint8_t i;
    KEY[0] = 0x1100;KEY[1] = 0x3322;
    KEY[2] = 0x5544;KEY[3] = 0x7766;
    KEY[4] = 0x9988;KEY[5] = 0xBBAA;
    KEY[6] = 0xDDCC;KEY[7] = 0xFFEE;
    for(i=0;i<16;i++)
        Plain[i] = i;
