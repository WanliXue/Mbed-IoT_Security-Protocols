...
// 256bit key size takes 84, and 256bit key value takes 92 constants
...
void KeySchedule256(unsigned char *RoundKey, const unsigned char *InitialKey)
{
    const unsigned char InitialConstant[2] = {0xb5U, 0xc0U}; // cubic root of 5 
    unsigned char IntermediateKey[32];
    unsigned char con256[4 * 92];
    int i;
    // Generate CONi^(256) (0 <= i < 92, IntermediateKey = 46) 
    ConstantGenerate(con256, InitialConstant, 46);
    // GFN(8,10) (generating IntermediateKey from InitialKey) 
    GFN_8(IntermediateKey, InitialKey, con256, 10);
    // Expanding InitalKey and IntermediateKey to generate RoundKey and WhiteningKey
    XOR(RoundKey, InitialKey, InitialKey + 16, 8); // InitialWhiteningKey (WK0, WK1)
    RoundKey += 8;
    for(i = 0; i < 13; i++)
    { 
        if((i / 2) % 2)
        {
            XOR(RoundKey, IntermediateKey + 16, con256 + i * 16 + (4 * 40), 16); // IntermediateKey Right
            if(i % 2)
            {
                XOR(RoundKey, RoundKey, InitialKey + 0,  16); //XOR K Left
            }
            DoubleSwap(IntermediateKey + 16); // Update IntermediateRight
        }
        else
        {
            XOR(RoundKey, IntermediateKey + 0,  con256 + i * 16 + (4 * 40), 16); // IntermediateKey Left
            if(i % 2)
            {
                XOR(RoundKey, RoundKey, InitialKey + 16, 16); //XOR Key Right
            }
            DoubleSwap(IntermediateKey + 0);  // Update IntermediateLeft
        }
        RoundKey += 16;
    }
    XOR(RoundKey, InitialKey + 8, InitialKey + 24, 8); // InitialWhiteningKey (WK2, WK3)
}
...
int main(void)
{
    ...
    int NoRounds = 26; //Number of rounds for 256Bit key
    ...
    {
        KeySchedule256(RoundKey, InitialKey);
