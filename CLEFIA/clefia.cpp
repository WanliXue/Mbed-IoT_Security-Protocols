...
int main(void)
{
  ...
  pc.printf("--- Test ---\n");
  pc.printf("plaintext:  "); BytePut(pt, 16);
  pc.printf("secretkey:  "); BytePut(skey, 32);

  /* for 128-bit key */
  pc.printf("--- CLEFIA-128 ---\n");
  /* encryption */
  r = ClefiaKeySet(rk, skey, 128);
  ClefiaEncrypt(dst, pt, rk, r);
  pc.printf("ciphertext: "); BytePut(dst, 16);
  /* decryption */
  ByteCpy(ct, dst, 16);
  r = ClefiaKeySet(rk, skey, 128);
  ClefiaDecrypt(dst, ct, rk, r);
  pc.printf("plaintext : "); BytePut(dst, 16);

  /* for 192-bit key */
  pc.printf("--- CLEFIA-192 ---\n");
  /* encryption */
